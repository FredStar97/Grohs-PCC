...
<<<<<<< Updated upstream
      -  2 Hidden layer structure: 2048 x 1024
=======
...
>>>>>>> Stashed changes
    ...
    max_epochs: int = 50      # Maximale Anzahl Epochen (harte Obergrenze)
...
=======
    mit LayerNorm, LeakyReLU und Dropout.
    """
    def __init__(self, input_dim: int, num_classes: int, cfg: IDPCollectiveFFNConfig):
        """
        - init: baut das Netzwerk
        - Layer 1: Input → 2048 (Linear, LayerNorm, LeakyReLU)
        - Layer 2: 2048 → 1024 (Linear, LayerNorm, LeakyReLU, Dropout)
        - Output: 1024 → Anzahl Deviation Types (Logits)
>>>>>>> Stashed changes
        """
        super().__init__()
...
=======
        # Output Layer: 1024 -> num_classes (Logits für alle Deviation Types)
>>>>>>> Stashed changes
...
=======
        ...
        Returns:
            Logits (noch keine Wahrscheinlichkeiten)
>>>>>>> Stashed changes
...
=======
        # Output: Logits (noch keine Wahrscheinlichkeiten)
>>>>>>> Stashed changes
        logits = self.head(x)
...
<<<<<<< Updated upstream
    ...
    # verwenden wir BCELoss mit Klassen-Gewichten
    # Die Gewichte β_d gleichen Klassen-Ungleichgewichte aus
    criterion = nn.BCELoss(weight=beta_d)
=======
...
<<<<<<< Updated upstream
            ...
            loss = criterion(probs, by)  # Loss berechnen
=======
            logits = model(bx)         # Forward Pass
            loss = criterion(logits, by)  # Loss berechnen
>>>>>>> Stashed changes
