...
def load_data(processed_dir: Path):
    ...
    if not probs_path.exists():
        ...
        if not probs_path.exists():
            raise FileNotFoundError(f"Datei nicht gefunden: {processed_dir / 'idp_collective_ffn_probs.npz'} oder {probs_path}. Bitte erst Collective FFN trainieren.")
    ...
    if len(case_ids) != P_dev_all.shape[0]:
        raise AssertionError(
            ...
            f"Bitte stelle sicher, dass case_ids in idp_collective_ffn_probs.npz gespeichert wurden."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluiert das Collective FFN-Modell"
    )
    parser.add_argument(
        "--dataset",
        type=str,
